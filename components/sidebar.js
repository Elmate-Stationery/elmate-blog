...
export function renderSidebar({ exclude = null } = {}) {
  ...
  const popularItems = popular
    .map(
      (b) => `
      <a href="blog.html?slug=${b.slug}" class="flex gap-3 group py-2.5 border-b rule last:border-0">
  ...
  const catItems = categories
    .map(
      (c) =>
        `<a href="category.html?slug=${c.slug}" class="flex items-center justify-between py-2 text-sm hover:text-[var(--pine)] transition-colors border-b rule last:border-0">
          <span>${c.name}</span>
  ...
  const tagChips = tags.map((t) => `<a href="tag.html?slug=${t.slug}" class="tag-chip">#${t.name}</a>`).join("");
