...
import { authorFor, categoryFor, formatDate, isBookmarked } from "../assets/js/utils.js";
    ...
    <article class="blog-card rounded-lg overflow-hidden flex flex-col sm:flex-row" data-aos="fade-up">
      ...
      <div class="p-5 flex flex-col gap-2 flex-1">
        <div class="flex items-center gap-2">
          ${category ? `<a href="category.html?slug=${category.slug}" class="badge">${category.name}</a>` : ""}
          ${blog.featured ? `<span class="badge badge-ochre">Featured</span>` : ""}
        </div>
        <h3 class="font-display text-lg font-semibold leading-snug">
          <a href="blog.html?slug=${blog.slug}" class="hover:text-[var(--pine)] transition-colors">${escapeHtml(blog.title)}</a>
        </h3>
  ...
  <article class="blog-card rounded-lg overflow-hidden flex flex-col h-full" data-aos="fade-up">
    ...
    <div class="p-5 flex flex-col gap-2 flex-1">
      <div class="flex items-center justify-between">
        ${category ? `<a href="category.html?slug=${category.slug}" class="badge">${category.name}</a>` : "<span></span>"}
      ...
      <div class="flex flex-wrap gap-1.5 mt-1">
        ${blog.tags
          .slice(0, 2)
          .map((tid) => `<span class="tag-chip">#${tid.replace("tag-", "")}</span>`)
          .join("")}
      </div>
